...
class XPCog(commands.Cog):
    ...
    async def level(self, ctx: discord.Interaction, user: discord.Member = None):
        ...
        pfpData.seek(0)
        iofile = config.createLevelImage(name=user.name, status=str(ctx.guild.get_member(user.id).status), rank=await self.client.getLevelLeaderboardIndex(user), level=user_level, percent=user_progress, expMin=userDoc['exp'],
